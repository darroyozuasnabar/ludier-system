        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50/60">
              ...
              {isAdmin && (
                <th className="text-right py-2.5 px-3 font-medium text-gray-500">Tarifa diaria</th>
              )}
          ...
          <tbody className="divide-y divide-gray-100">
            {data.map((w, i) => (
              <tr key={`${w.name}-${i}`} className="hover:bg-gray-50/60 transition-colors">
                ...
                {isAdmin && (
                  <td className="py-2.5 px-3 text-right text-gray-900 font-medium tabular-nums">
                    S/ {Number(w.tarifa_diaria || 0).toLocaleString("es-PE", { minimumFractionDigits: 2 })}
                  </td>
                )}
